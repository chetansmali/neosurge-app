...
class LumpsumStepper extends HookWidget {
  final MfOrder mfOrder;

  const LumpsumStepper({Key? key, required this.mfOrder}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return EnhanceStepper(
      ...
      currentStep: (mfOrder.submittedAt == null || mfOrder.status == MFOrderStatus.failed) ? 1 : 2,
      steps: [
        EnhanceStep(
          title: Text(
            'Order Placed on Neosurge',
        ...
        if (mfOrder.status == MFOrderStatus.failed) ...[
          EnhanceStep(
            ...
            content: const SizedBox.shrink(),
            subtitle: Text(df.format(mfOrder.failedAt!.toLocal()), style: OrderStyles.finishedStepSubtitleStyleLight),
            icon: const Icon(
        ...
        //If we don't have submittedAt date it means payment is processing
          ...
          EnhanceStep(
            title: Text(
              'Payment Processing',
              style: OrderStyles.unfinishedStepTitleStyleLight,
            ),
            content: const SizedBox.shrink(),
            icon: const Icon(
        ...
        //If we have submittedAt date, it means payment is successful
          ...
          EnhanceStep(
            ...
            subtitle: Text(df.format(mfOrder.submittedAt!.toLocal()), style: OrderStyles.finishedStepSubtitleStyleLight),
            content: const SizedBox.shrink(),
            icon: const Icon(
              Icons.check_circle_outline,
              color: AppColors.green,
          ...
          mfOrder.status == MFOrderStatus.pending
              ? EnhanceStep(
                  ...
                  subtitle: Text('Expect by ${df2.format(mfOrder.submittedAt!.add(const Duration(days: 3)))}', style: OrderStyles.unfinishedStepSubtitleStyleLight),
                  content: const SizedBox.shrink(),
                  icon: const Icon(
                    Icons.schedule_outlined,
                    color: AppColors.darkOrange,
              ...
              : EnhanceStep(
                  ...
                  subtitle: Text(df.format(mfOrder.succeededAt!.toLocal()), style: OrderStyles.finishedStepSubtitleStyleLight),
                  content: const SizedBox.shrink(),
                  icon: const Icon(
                    Icons.check_circle_outline,
                    color: AppColors.green,
      ...
      controlsBuilder: (context, details) => mfOrder.status == MFOrderStatus.pending && mfOrder.submittedAt != null
          ? Container(
