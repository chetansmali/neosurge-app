...
class SwitchStepper extends HookWidget {
  final MfOrder mfOrder;

  const SwitchStepper({Key? key, required this.mfOrder}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return EnhanceStepper(
      currentStep: mfOrder.status == MFOrderStatus.failed ? 0 : 1,
      steps: [
        EnhanceStep(
          title: Text(
            'Switch Request Placed on Neosurge',
        ...
        if (mfOrder.status == MFOrderStatus.failed) ...[
          ...
          const EnhanceStep(title: Text('Switch request processing'), content: SizedBox.shrink(), icon: Icon(Icons.schedule_rounded, color: AppColors.darkOrange)),
        ] else ...[
          EnhanceStep(
            title: const Text('Switch Successful'),
