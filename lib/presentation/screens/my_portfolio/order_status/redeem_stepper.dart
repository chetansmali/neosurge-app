...
class RedeemStepper extends HookWidget {
  final MfOrder mfOrder;

  const RedeemStepper({Key? key, required this.mfOrder}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return EnhanceStepper(
      ...
      currentStep: (mfOrder.submittedAt == null || mfOrder.status == MFOrderStatus.failed) ? 1 : 1,
      steps: [
        EnhanceStep(
          title: Text(
            'Redeem order placed',
        ...
        if (mfOrder.status == MFOrderStatus.failed) ...[
          ...
          const EnhanceStep(title: Text('Redeem request processing'), content: SizedBox.shrink(), icon: Icon(Icons.schedule_rounded, color: AppColors.darkOrange)),
        ] else ...[
          EnhanceStep(
