...
class StpDetailScreen extends StatelessWidget {
  final SystematicPlan stp;

  const StpDetailScreen({Key? key, required this.stp}) : super(key: key);
      ...
      child: BlocListener<CancelSystematicPlanCubit, CancelSystematicPlanState>(
        listener: (context, state) {
          if (state is CancelSystematicPlanSuccess) {
            ...
            context.read<SystematicPlansCubit>().cancelSystematicPlans(stp.id);
          }
        ...
        child: Scaffold(
          appBar: AppBar(
            title: const Text('Stp Details'),
            actions: [
              if (stp.status == SystematicPlansStatus.activated)
                          ...
                          onPressed: () {
                            showDialog(
                                ...
                                builder: (context) => AlertDialog(
                                      ...
                                      content: const Text('This will cancel the STP'),
                                      actions: [
                                        TextButton(
                              ...
                              if (value == true) {
                                context.read<CancelSystematicPlanCubit>().cancelStp(userId: context.read<AuthCubit>().state.user!.id, stpId: stp.id);
                              }
                            });
                          },
              ...
              if (state is SystematicPlanInstallmentsError) {
                return Center(
                  child: AppErrorWidget(
                    ...
                    error: state.error,
                    onRetry: () => context.read<SystematicPlanInstallmentsCubit>().fetchStpInstallments(userId: context.read<AuthCubit>().state.user!.id, stpId: stp.orderId!),
                  ),
                );
              }
              if (state is SystematicPlanInstallmentsLoaded) {
                    ...
                    children: [
                      Card(
                        child: Column(
                          children: [
                            ListTile(
                              leading: CachedNetworkImage(imageUrl: stp.imgUrl, width: 50, height: 50),
                              title: Text(
                                stp.plan,
                            ...
                            if (stp.status == SystematicPlansStatus.activated) ...[
                              ...
                              ListTile(
                                title: Text(
                                  'Next due on ${DateFormat('dd MMM yyyy').format(stp.nextInstallmentDate)}',
                                ...
                                trailing: Row(
                                  ...
                                  children: [
                                    Text(
                                      NumberFormat.currency(locale: 'en_IN', symbol: '\u20b9', decimalDigits: 0).format(stp.amount),
                      ...
                      EnhanceStepper(
                        ...
                        steps: state.installments[0].succeededAt == null
                            ? [
                                stp.status == SystematicPlansStatus.deactivated
                                    ? EnhanceStep(
                                        title: const Text('STP Cancelled on'),
                                        icon: const Icon(
                                          Icons.close,
                                          color: AppColors.errorRed,
                                        ),
                                        content: const SizedBox.shrink(),
                                        subtitle: Text(DateFormat('dd MMM yyyy hh:mm a').format(stp.updatedAt.toLocal())),
                                      )
                                    : EnhanceStep(
                                        title: Text('${Utils.suffixText(1)} STP in Progress'),
                                        icon: const Icon(
                            ...
                            : List.generate(
                                ...
                                (index) {
                                  if (index == 0) {
                                    if (stp.status == SystematicPlansStatus.deactivated) {
                                      return EnhanceStep(
                                        title: const Text('STP Cancelled on'),
                                        icon: const Icon(
                                          Icons.close,
                                    ...
                                    return EnhanceStep(
                                      title: Text('${Utils.suffixText(state.installments.length + 1)} STP due in'),
                                      icon: const Icon(
                                        Icons.schedule_outlined,
                                        color: AppColors.darkOrange,
                                      ),
                                      content: const SizedBox.shrink(),
                                      subtitle: Text('${stp.startDate.difference(DateTime.now()).inDays} days'),
                                    );
                                  }
                                  return EnhanceStep(
                                    title: Text('${Utils.suffixText(index)} STP processed on'),
                                    icon: const Icon(
